...
namespace MDC
{
    public partial class frmNGScan : Form
    {
        #region 私有字段
        /// <summary>
        /// 数据上传客户端
        ...
        /// 仅能申报摄像孔脏污不良
        /// </summary>
        private bool isCameraOnly = false;
        /// <summary>
        ...
        private void frmManualScan_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                // 仅能申报摄像孔脏污不良
                this.isCameraOnly = BaseUI.UI_RIGHT.ContainsKey("02121202");
        ...
        private void Initialize()
        {
            ...
            //this.ipAddress = "172.16.4.122";    //	LCD清洗投入机	
...
//            }
//            //txtIPAddress.Text = "192.168.23.159";
//#endif
            // 上报服务器端口号
        ...
        private void BindProcessComboBox(string ProductionOrder, string LineCode)
        {
            try
            {
                ...
                {
                    DataView dvPro = new DataView(this.dtProcessTable);
                    dvPro.RowFilter = "SGX_JobCode IN ('040', '034')";
                    dvPro.Sort = "PR_NoSeq ASC";
                    if (dvPro.Count == 0)
                    {
                        ShowResult(NoteState.Error, "", "当前产线工单未配置FV2正/背面工序，无法申报摄像孔脏污不良。");
                    }
                    else
                    {
                        GXCode = dvPro[0]["SGX_JobCode"];
                ...
                {
                    // 设置程序标题
                    this.Text = "不良批次申报 --【请选择本站工序】   版本:" + this.versionName + "（技术支持：深圳市鼎立特科技有限公司）";
                    // 补扫站点，待选择
                    comProcess.SelectedIndex = -1;
                }
                else
            ...
            try
            {
                ...
                else
                {
                    ...
                    switch (state)
                    {
                        ...
                        case NoteState.OK:
                            // 弹框提示
                            this.successMsg = new SuccessMessage("不良申报成功!", 2);
        ...
        private void PageInit()
        {
            ...
            try
            {
                ...
                if (this.dvProcess.Count > 0)
                {
                    ...
                    BaseUI.UI_CurrentProcedureName = this.dvProcess[0]["SGX_Name"].ToString();      //当前工序名称

                    //当前站点IP地址,是获取的工序对应IP
                    BaseUI.UI_EquCardIP = this.dvProcess[0]["STW_CardIP"].ToString();
                    BaseUI.UI_EquDeviceCode = this.dvProcess[0]["STW_JobCode"].ToString();
                    ...
                    // 设置程序标题
                    this.Text = "不良批次申报 -- " + this.dvProcess[0]["SGX_Name"].ToString() + "   版本:" + this.versionName + "（技术支持：深圳市鼎立特科技有限公司）";
            ...
            try
            {
                ...
                if (IsScan)
                {
                    ...
                    else
                    {
                        ...
                        {
                            ShowResult(NoteState.Fail, "", "请选择不良项！");
                            return;
                        }
                        ...
                        switch (rst.State)
                        {
                            ...
                            case NoteState.OK:
                                //#region 发送过站数据
                                //// 生成发送字符串
                                //string sendCode = GetSendString("1", txtDescribe.Text);
                                //if (sendCode == "")
                                //{
        ...
        private NoteResult isValidCode1(string DataString)
        {
            ...
            if (this.glassInfo.ProductInfo != null)
            {
                ...
                if (this.glassInfo.GlassState != null)
                {
                    ...
                    if (Isrepeat && !this.processRepeatEnabled && !BaseUI.UI_IsBind && BaseUI.UI_CurrentProcedureNo != "1")
                    {
                        // 如果非关键工序，且禁止返检，提示错误信息
                        string msg = "当前工序已过站,\r\n请勿重复扫描!";
                        IsScan = false;
                        return new NoteResult(NoteState.NG, DataString, msg);
                    }
        ...
        private void InitBNC()
        {
            ...
            if (txtDescribe.Text != "")
            {
                ...
                keys = txtDescribe.Tag.ToString();
            }

            txtDescribe.Clear();
            flpNG.Controls.Clear();
            // 添加不良项列表
            Dictionary<int, string> BNCList;
            if (this.isCameraOnly)
            {
                BNCList = new Dictionary<int, string>()
                {
                    { 1559, "摄像孔脏污"}
                };
            }
            ...
            {
                BNCList = BaseUI.GetDefaultBNCItem(BaseUI.UI_CurrentProcedureCode);
            }
            //int width = 0;
            ...
            foreach (int key in BNCList.Keys)
            {
                CheckBox chk = new CheckBox()
                {
                    ...
                    Height = 45,
                    AutoEllipsis = true,
                    Font = new System.Drawing.Font("微软雅黑", 18F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((byte)(134))),
            ...
            if (this.flpNG.Controls.Count > 0)
            {
                if (describe != "")
                {
                    ...
                    {
                        if (this.flpNG.Controls.ContainsKey(id))
                        {
                            (this.flpNG.Controls[id] as CheckBox).Checked = true; 
                        }
                    }
                }
                else
        ...
        private void bgwWriteData_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                ...
                // 获取不良申报时间（服务器时间）
                DateTime addTime = BaseUI.GetServerTime();
                
                // 判断是否已过当前工序
                bool Isrepeat = BaseUI.CheckProcedure(this.glassInfo.GlassState.LogCode);
                // 如果是关键工序返检过站，则需直接操作HBase
                if (Isrepeat && (BaseUI.UI_IsBind || BaseUI.UI_CurrentProcedureNo == "1"))
                {
                    // 确定申报时间段（1：前半小时，2：后半小时）
                    int part = addTime.Minute < 30 ? 1 : 2;
                    // lotNumber
                    string lotNumber = string.Format("{0}{1}1{2}", BaseUI.UI_EquDeviceCode, addTime.ToString("yyyyMMddHH"), part);
                    // lotNumberSecond,exception_05的RowKey
                    string lotNumberSecond = string.Format("{0}{1}{2}1{3}", BaseUI.UI_SPLJOBCODE, BaseUI.UI_CurrentProcedureCode, addTime.ToString("yyyyMMddHH"), part);

                    if (this.glassInfo.Exception == null)
                    {
                    ...
                    {
                        ExceptionKey = string.Format("$G{0}$T{1}$P{2}", lcd, addTime.ToString("yyyyMMddHHmmss"), BaseUI.UI_CurrentProcedureCode),
                        GlassCode = lcd,
                        ProductionOrder = order,
                        FinishesCode = this.glassInfo.ProductInfo.FinishesCode,
                        FinishesModel = this.glassInfo.ProductInfo.FinishesModel,
                        ProductionLineCode = BaseUI.UI_SPLJOBCODE,
                        ProductionLineName = BaseUI.UI_SPLNAME,
                        ProcessCode = BaseUI.UI_CurrentProcedureCode,
                        processName = BaseUI.UI_CurrentProcedureName,
                        ScanNumber = BaseUI.UI_BOOLOGNAME,
                        ScanTime = addTime.ToString("yyyy-MM-dd HH:mm:ss"),
                        DeviceIp = this.ipAddress,
                        ExceptionContent = bncname,
                        ExceptionState = "待复判",
                        LotNumber = lotNumber,
                        LotNumberSecond = lotNumberSecond,
                        RepeatStatus = "0",
                        ReworkStatus = "1",
                    };
                    this.glassInfo.Exception.Add(exInfo.ExceptionKey, exInfo);

                    this.glassInfo.LastExceptionKey = exInfo.ExceptionKey;

                    // 调用HBase方法进行不良申报
                    string result = GetHBaseDataClass.Instance.ExceptionAdd(ref this.glassInfo);

                    if (result.Contains("失败"))
                    {
                        e.Result = new NoteResult(NoteState.NG, code, result);
                        return;
                    }
                    else
                    {
                        rst = new NoteResult(NoteState.OK, code, "返检不良申报成功!");
                ...
                // 调用存储过程，提交数据
                bool isAdded = BaseUI.Add_ProcessFail("正操作", "重工", code, lcd, addTime.ToString("yyyy-MM-dd HH:mm:ss"), bncid, bncname, "01", ip, pid, order);
                if (isAdded)
                {
                    rst = new NoteResult(NoteState.OK, code, "不良描述：" + bncname);
                    e.Result = rst;
                }
            }
            catch (Exception ex)
        ...
        private void txtProcessIP_TextChanged(object sender, EventArgs e)
        {
