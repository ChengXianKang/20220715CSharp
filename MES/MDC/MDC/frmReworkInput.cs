...
namespace MDC
{
    public partial class frmReworkInput : Form
    {
        #region 私有字段
        /// <summary>
        /// SQL Server数据库操作类
        ...
        /// <param name="e"></param>
        private void frmReworkInput_Load(object sender, EventArgs e)
        {
            try
            {
                pnlNG.Visible = false;
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frmReworkInput_Shown(object sender, EventArgs e)
        {
            //// 打开串口
            //OpenDeviceCom();
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frmReworkInput_FormClosed(object sender, FormClosedEventArgs e)
        {
            Program.ScanDevice.DataReceived -= this.ScanDevice_DataReceived;
            //CloseDeviceCom();
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frmReworkInput_SizeChanged(object sender, EventArgs e)
        {
            this.Refresh();
        }
        /// <summary>
...
#if DEBUG
            //this.ipAddress = "172.16.28.86";    //	CELL投入
            //this.ipAddress = "172.16.14.226";    //	LCD清洗投入机	
            //this.ipAddress = "172.16.4.167";    //71线FOG
            //this.ipAddress = "172.16.4.168";    //	TP贴合（工控机）	
            //this.ipAddress = "172.16.4.140";    //	FV1(人工)	
            //this.ipAddress = "172.168.1.14";    //	成品码生成（喷码）
            //this.ipAddress = "172.168.1.112";    //	71线包装2	
            //this.ipAddress = "172.16.9.121";    //	机台补发51线
            //this.ipAddress = "172.16.4.149";    //	机台补发32线
            //this.ipAddress = "172.10.4.104";    
#endif
...
//#endif
                        ...
                        switch (rst.State)
                        {
                            ...
                            case NoteState.OK:
                                ...
                                else
                                {
                                    ...
                                    bgwReworkInput.RunWorkerAsync(new object[] { AnalysisCode, this.glassInfo, txtProcessIP.Text, YJ.Common.TypeConverter.StrToInt(BaseUI.UI_BOOPID), txtOrder.Text, BaseUI.UI_SPLJOBCODE, txtOpCode.Text, reworkTime });
        ...
        private NoteResult isValidCode(string DataString)
        {
            txtglassCode.Text = "";
            this.glassInfo = null;
            this.isRepeat = false;
            ...
            #region 编码合法性检测
            // 检查是否获取玻璃信息
            if (this.glassInfo.ProductInfo == null || string.IsNullOrWhiteSpace(this.glassInfo.ProductInfo.LCDCode))
            {
                IsScan = false;
            ...
            {
                IsScan = false;
                return new NoteResult(NoteState.Error, DataString, "该玻璃型号为" + this.glassInfo.ProductInfo.FinishesModel + ",与当前工单不一致。");
            }
            #endregion 型号切换检测
            ...
            //{
            //    IsScan = false;
            //    return new NoteResult(NoteState.Fail, DataString, "该玻璃未包装，无法重工投入！");
            //}
            #endregion 判断玻璃是否已包装
        ...
        private NoteResult isValidCode1(string DataString)
        {
            ...
            //    {
            //        IsScan = false;
            //        return new NoteResult(NoteState.Error, DataString, "该玻璃生产产线为" + lineName + ",禁止在当前产线生产。");
            //    }
            //    //// 不同型号禁止混线
            ...
            //        {
            //            IsScan = false;
            //            return new NoteResult(NoteState.Error, DataString, "该玻璃成品料维护中未配置可混产线。");
            //        }
            ...
            if (this.glassInfo.ProductInfo != null)
            {
                ...
                //// 从不良记录表TPL_ProcessFail_Log查询玻璃最近的不良状态
                ...
                //// 如果lcdState为空或者良品，则以SQL中的不良状态为准
                //if (string.IsNullOrWhiteSpace(this.glassInfo.GlassState.LCDState) || this.glassInfo.GlassState.LCDState == "良品")
                //{
                //    this.glassInfo.GlassState.LCDState = reviewResult.ToString();
                //}
                //else if (!this.glassInfo.GlassState.LCDState.Contains("报废") && (reviewResult == ExceptionState.复判报废 || reviewResult == ExceptionState.重工报废))
                //{
                //    this.glassInfo.GlassState.LCDState = reviewResult.ToString();
                //}
                //else if(this.glassInfo.GlassState.LCDState == "复判良品" && (reviewResult != ExceptionState.良品 && reviewResult != ExceptionState.复判良品))
                //{
                //    this.glassInfo.GlassState.LCDState = reviewResult.ToString();
                //}
                //else if (this.glassInfo.GlassState.LCDState == "重工良品" && (reviewResult != ExceptionState.良品 && reviewResult != ExceptionState.复判良品))
                //{
                ...
                //    // HBase为重工良品，但是SQL不是良品，则需判断是否同一次不良已重工
                //    this.glassInfo.Exception = GetHBaseDataClass.Instance.GetException(this.glassInfo.ProductInfo.LCDCode);
                //    if (this.glassInfo.Exception == null || this.glassInfo.Exception.Count == 0)
                //    {
                //        this.glassInfo.GlassState.LCDState = reviewResult.ToString();
                //    }
                //    else if (ProcessFailInfo != null)
                //    {
                //        // 获取最近时间的不良记录
                //        this.glassInfo.LastExceptionKey = null;
                //        foreach (string key in this.glassInfo.Exception.Keys)
                ...
                //        // 确认是否重工良品新提报的不良
                //        if (lastEx.ExceptionState == "重工良品" && DateTime.Compare(DateTime.Parse(lastEx.ReworkTime), (DateTime)ProcessFailInfo.TFM_AddDate) < 0)
                //        {
                //            this.glassInfo.GlassState.LCDState = reviewResult.ToString();
                //        }
                //    }
                        ...
                        case "复判良品":
                            ...
                            if (!string.IsNullOrWhiteSpace(this.warnType) && this.warnType.Substring(1, 1) == "1")
                            {
                                ...
                                else
                                {
                                    ...
                                    if (!string.IsNullOrWhiteSpace(this.warnType) && this.warnType.Substring(2, 1) == "1" && lastEx.ProcessCode == "031" && lastEx.ExceptionState == "复判良品" && lastEx.ExceptionContent.Contains("点"))
                                    {
        ...
        private void bgwWriteData_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                ...
                if (sendCode == "")
                {
                    rst = new NoteResult(NoteState.NG, txtCode1.Text, "生成发送字符串失败!");
                    e.Result = rst;
                    return;
                }
                ...
                if (isAdded)
                {
                    rst = new NoteResult(NoteState.OK, txtCode1.Text, "不良申报成功，不良描述：" + txtDescribe.Text);
                    e.Result = rst;
                }
                else
                {
                    rst = new NoteResult(NoteState.NG, txtCode1.Text, "不良信息提交失败!");
                    e.Result = rst;
                }
            ...
            catch (Exception ex)
            {
                ...
                e.Result = new NoteResult(NoteState.NG, txtCode1.Text, "不良信息提交失败!" + ex.Message);
            }
        }
        ...
        #region 客退重工投入
        private void bgwReworkInput_DoWork(object sender, DoWorkEventArgs e)
        {
            NoteResult rst;
            try
            {
                ...
                string code = val[0].ToString();
                GlassInfo glassInfo = val[1] as GlassInfo;
                string ip = val[2].ToString();
                int pid = Convert.ToInt32(val[3]);
                string order = val[4].ToString();
                string line = val[5].ToString();
                string op = val[6].ToString();
                string time = val[7].ToString();
                #endregion 传参
                ...
                {
                    // 解除包装信息
                    string sql = string.Format("EXEC TPL_GlassUnpack_Pro '{0}','{1}','{2}'", glassInfo.ProductInfo.LCDCode, ip, pid);
                    conn.ExecuteAction(sql, "Base");
                    rst = new NoteResult(NoteState.Success, code, "已解除包装信息.");
                    bgwReworkInput.ReportProgress(20, rst);
                }
                catch (Exception exp)
                {
                    LogHelper.Error(exp.Message, exp);
                    e.Result = new NoteResult(NoteState.NG, code, "重工投入失败." + exp.Message.ToString());
                    return;
                }
                #endregion 解除包装信息

                #region 更新HBase
                string str = GetHBaseDataClass.Instance.ReworkInput(this.glassInfo, order, line, op, time, ip);
                if (str != "成功")
                {
                    e.Result = new NoteResult(NoteState.NG, txtCode1.Text, "重工投入提交失败!");
                    return;
                }
                else
                {
                    rst = new NoteResult(NoteState.Success, code, "已解除绑定条码.");
                    bgwReworkInput.ReportProgress(50, rst);
                }
                #endregion 更新HBase
                ...
                e.Result = new NoteResult(NoteState.OK, txtCode1.Text, "重工投入成功!"); ;
                return;
            }
            catch (Exception ex)
            {
                LogHelper.Error("重工投入提交失败!", ex);
                e.Result = new NoteResult(NoteState.NG, txtCode1.Text, "重工投入提交失败!" + ex.Message);
            }
        }
        private void bgwReworkInput_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            if (e.UserState == null) return;
            NoteResult rst = e.UserState as NoteResult;
            if (rst.State == NoteState.OK)
        ...
        private void bgwReworkInput_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            if (e.Result != null)
            {
                NoteResult rst = e.Result as NoteResult;
