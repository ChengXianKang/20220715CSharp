...
namespace MDC
{
    public partial class frmAutoPackage : Form
    {
        #region 私有字段
        /// <summary>
        /// 数据上传客户端
        ...
        /// 自动包装串口
        /// </summary>
        private SerialPortUtil ComPackage = new SerialPortUtil("", SerialPortBaudRates.BaudRate_9600, Parity.None, SerialPortDatabits.EightBits, StopBits.One);
        /// <summary>
        /// 相邻工序过站间隔时间
        /// </summary>
        ...
        /// <param name="e"></param>
        private void frmAutoPackage_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                this.ComPackage.DataReceived += new Utils.DataReceivedEventHandler(ComPackage_DataReceived);
                // 程序初始化
                Initialize();
            }
            catch (Exception ex)
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frmAutoPackage_Shown(object sender, EventArgs e)
        {
            //// 打开串口
            //OpenDeviceCom();
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frmAutoPackage_FormClosed(object sender, FormClosedEventArgs e)
        {
            Program.ScanDevice.DataReceived -= this.ScanDevice_DataReceived;
            this.ComPackage.ClosePort();
            this.ComPackage.Dispose();
            //CloseDeviceCom();
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frmAutoPackage_SizeChanged(object sender, EventArgs e)
        {
            this.Refresh();
        }
        /// <summary>
        ...
        private void Initialize()
        {
            ...
            if (BaseUI.UI_SGXJOBCODE != "039")
            {
                throw new Exception("本机不属于成品码绑定工序！");
            }
...
//#endif
            ...
            // 初始化包装串口
            if (appConfig.ContainsKey("PackagePort"))
            {
                string com = appConfig.GetConfigString("PackagePort");
                txtComPackage.Text = com;
                if (Program.ScanDevice.portList.ContainsKey(com))
                {
                    Program.ScanDevice.ClosePort(com);
                }
                try
                {
                    ComPackage.PortName = com;
                    ComPackage.OpenPort();

                    if (!isPackageReady())
                    {
                        throw new Exception("包装电脑未连接!");
                        //ShowResult(NoteState.NG, this.ComPackage.PortName, "包装电脑未连接!");
                        //return;
                    }
                }
                catch (Exception ex)
                {
                    LogHelper.Error("自动包装串口初始化失败.", ex);
                    throw new Exception("请正确配置包装串口名PackagePort");
                }
            }
            else
            {
                appConfig.SetConfig("PackagePort", "");
                throw new Exception("请先在配置文件中设置包装串口PackagePort");
            }
        ...
        private void ComPackage_DataReceived(Utils.DataReceivedEventArgs e)
        {
                        ...
                        switch (rst.State)
                        {
                            ...
                            case NoteState.OK:
                                ...
                                if (rst.State == NoteState.OK)
                                {
                                    ...
                                    string qrcode = txtCode2.Text;
                                    System.Threading.Thread thread = new Thread(new ParameterizedThreadStart(SendToPackage));
                                    thread.Name = qrcode + "|" + DateTime.Now.ToString("HH:mm:ss");
                                ...
                                //NoteResult rstPackage = null;
                                //// 检测是否绑定成功
                                //rstPackage = isQrCodeBinded();
                                //if (rstPackage.State != NoteState.OK)
                                //{
                                //    ShowResult(rstPackage);
                                //    return;
                                //}

                                //if (!sendPackageData())
                                //{
                                //    ShowResult(NoteState.NG, txtCode2.Text, "包装失败！\n请重试.");
                                //    return;
                                //}
        ...
        /// <summary>
        /// 异步检测成品码是否绑定，并发送给包装电脑
        /// </summary>
        /// <param name="qrCode"></param>
        private void SendToPackage(object qrCode)
        {
            string code = qrCode.ToString();
            NoteResult rstPackage = null;
            // 检测是否绑定成功
            rstPackage = isQrCodeBinded(code);
            if (rstPackage.State != NoteState.OK)
            {
                ShowResult(rstPackage);
                return;
            ...
            if (!sendPackageData())
            {
                ShowResult(NoteState.NG, code, code + "\n包装失败,请重试!");
                return;
            }
        }
        ...
        private bool isPackageReady()
        {
            ...
            ComPackage.SendCommand(new UTF8Encoding().GetBytes("09"), ref data, 1000);
            if (new UTF8Encoding().GetString(data) != "10")
            {
                return false;
            }

            ComPackage.SendCommand(new UTF8Encoding().GetBytes("11"), ref data, 1000);
            if (new UTF8Encoding().GetString(data) != "12")
            {
                return false;
            }
            return true;
        ...
        /// </summary>
        /// <returns></returns>
        private bool sendPackageData()
        {
            byte[] code = new UTF8Encoding().GetBytes(txtCode2.Text.Trim());
            byte[] data = new byte[2];
            ComPackage.SendCommand(code, ref data, 3000);
            if (new UTF8Encoding().GetString(data) != "13")
            {
                return false;
            }
            return true;
        ...
        /// </summary>
        /// <returns></returns>
        private NoteResult isQrCodeBinded(string qrCode)
        {
            ...
            while (!istimeout)
            {
                ...
                try
                {
                    bindData = GetHBaseDataClass.Instance.GetProductInfo(qrCode);//Code2查询结果
                    if (bindData == null)
                    {
                        return new NoteResult(NoteState.Fail, qrCode, "成品码绑定失败.");
                    }
                ...
                {
                    IsScan = false;
                    return new NoteResult(NoteState.Fail, qrCode, "成品码绑定失败." + exp.Message.ToString());
                }
        ...
        private NoteResult isValidCode1(string DataString)
        {
            ...
            try
            {
                ...
                {
                    IsScan = false;
                    return new NoteResult(NoteState.Fail, DataString, "查询玻璃信息失败,如多次出现此错误，请关掉程序重新打开");
                }
            }
            catch (TimeoutException tex)
            {
            ...
            //        {
            //            IsScan = false;
            //            return new NoteResult(NoteState.Error, DataString, "该玻璃工单未配置可混产线。");
            //        }
            ...
            {
                IsScan = false;
                return new NoteResult(NoteState.NG, DataString, "该玻璃已包装，无法过站！");
            }
            #endregion 判断玻璃是否已包装
