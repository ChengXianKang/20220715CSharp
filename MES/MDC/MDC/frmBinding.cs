...
namespace MDC
{
    public partial class frmBinding : Form
    {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frmBinding_Shown(object sender, EventArgs e)
        {
            // 连接服务器
            ClientStart();
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frmBinding_FormClosed(object sender, FormClosedEventArgs e)
        {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frmBinding_SizeChanged(object sender, EventArgs e)
        {
            this.Refresh();
        }
        /// <summary>
        ...
        private void Initialize()
        {
            ...
            //this.ipAddress = "172.20.23.133";   //机台补发
            this.ipAddress = "172.20.21.46";   
#endif
...
//#endif
        ...
        /// 处理绑定数据的委托
        /// </summary>
        /// <param name="code">已绑定编码</param>
        private delegate void BindingHandlerCallback(string code);
        /// <summary>
        /// 处理绑定数据
        /// </summary>
        /// <param name="code">已绑定编码</param>
        private void BindingHandler(string code)
        {
            try
            {
                // InvokeRequired required compares the thread ID of the
                ...
                if (IsScan)
                {
                    if (this.InvokeRequired)
                    {
                        BindingHandlerCallback d = new BindingHandlerCallback(BindingHandler);
                        this.BeginInvoke(d, new object[] { code });
                    }
                    else
                    {
                        txtScanCode.Text = code;  // 实际扫描数据
                        code = code.Replace("|", "/").Replace(";", "；").Replace(" ", "");
                        string code1, code2;
                        try
                        {
                            //反序列化rowValue
                            JObject jsonStr = JObject.Parse(code);
                            code1 = (jsonStr["code1"] ?? "").ToString();
                            code2 = (jsonStr["code2"] ?? "").ToString();
                        }
                        catch (Exception ex)
                        {
                            LogHelper.Error(txtScanCode.Text + "|绑定条码数据格式异常。" + ex.Message, ex);
                            ShowResult(NoteState.NG, txtScanCode.Text, "绑定条码数据格式异常." + ex.Message);
                            return;
                        }
                        ...
                        // 第2步：解析扫描数据
                        #region 第2步：解析扫描数据
                        if (code1=="NG" || code1 == "ERROR")
                        {
                            ShowResult(NoteState.NG, txtScanCode.Text, this.scanType1 + "漏扫,编码绑定失败!");
                            return;
                        }
                        if (code2 == "NG" || code2 == "ERROR")
                        {
                            ShowResult(NoteState.NG, txtScanCode.Text, this.scanType2 + "漏扫,编码绑定失败!");
                            return;
                        }
                        // 小写字母检查
                        if (BaseUI.LowerCheck(code1) || BaseUI.LowerCheck(code2))
                        {
                            if (appConfig.ContainsKey("LowerEnabled"))
                            {
                                if (!appConfig.GetConfigBool("LowerEnabled"))
                                {
                                    ShowResult(NoteState.Fail, txtScanCode.Text, "条码中禁止包含小写字母");
                                    return;
                                }
                            }
                            else
                        ...
                        // 判断是否合法的Code1
                        this.AnalysisCode = code1;
                        rst = isValidCode1(code1);
                        #endregion 第3步：判断是否合法的Code1

                        // 第4步：判断是否合法的Code2（如果有）
                        #region 第4步：判断是否合法的Code2（如果有）
                        if (rst.State == NoteState.Success && txtCode2.Visible && comProcess.SelectedIndex != -1)
                        {
                            // 判断是否合法的Code2
                            this.AnalysisCode = code2;
                            rst = isValidCode2(code2);
                        }
                        #endregion 第4步：判断是否合法的Code2（如果有）

                        // 第5步：输出检测结果,发送数据到服务器
                        #region 第5步：输出检测结果,发送数据到服务器
                        rst.Code = txtScanCode.Text;
                        switch (rst.State)
                        {
        ...
        private NoteResult isValidCode1(string DataString)
        {
            ...
            if (this.glassInfo.ProductInfo != null && !string.IsNullOrEmpty(this.glassInfo.ProductInfo.ProductionOrder) && this.glassInfo.ProductInfo.ProductionOrder != txtOrder.Text)
            {
                ...
                {
                    return new NoteResult(NoteState.Error, DataString, "该玻璃工单无本站点工序");
                }
            }
        ...
        private string GetSendString(string NGState, string NGDesc)
        {
            ...
            switch (this.siteType)
            {
                ...
                case SiteType.过站扫描点:
                    sendString = "RGJT{RGZD;" + txtStartTime.Text + ";" + txtProcessIP.Text + ";" + txtglassCode.Text.Trim() + ";" + txtCode1.Text.Trim() + "|" + txtCode1.Text.Trim() + ";" + txtOpCode.Text.Trim() + ";" + NGState + ";" + NGDesc + "}";
                    break;

                // 关键扫描点
                case SiteType.关键扫描点:
                    sendString = "RGJT{RGZD;" + txtStartTime.Text + ";" + txtProcessIP.Text + ";" + txtglassCode.Text.Trim() + ";" + txtCode1.Text.Trim() + "|" + txtCode2.Text.Trim() + ";" + txtOpCode.Text.Trim() + ";" + NGState + ";" + NGDesc + "}";
                    break;
                ...
                default:
                    ...
                    if (this.isBind)
                    {
                        ...
                        {
                            sendString = "RGJT{JTBF;" + txtStartTime.Text + ";" + txtProcessIP.Text + ";;" + txtCode1.Text.Trim() + "|" + txtCode2.Text.Trim() + ";" + txtOpCode.Text.Trim() + ";TF;" + NGState + ";" + NGDesc + "}";
                        }
                        else if (txtProcessCode.Text == "032")//54位成品喷码补发
                        {
                            sendString = "RGJT{JTBF;" + txtStartTime.Text + ";" + txtProcessIP.Text + ";;" + txtCode1.Text.Trim() + "|" + txtCode2.Text.Trim() + ";" + txtOpCode.Text.Trim() + ";CP;" + NGState + ";" + NGDesc + "}";
                        }
                        else
                        {
                            sendString = "RGJT{JTBF;" + txtStartTime.Text + ";" + txtProcessIP.Text + ";;" + txtCode1.Text.Trim() + "|" + txtCode2.Text.Trim() + ";" + txtOpCode.Text.Trim() + ";#;" + NGState + ";" + NGDesc + "}";
                        }
                    ...
                    else
                    {
                        ...
                        sendString = "RGJT{JTBF;" + txtStartTime.Text + ";" + txtProcessIP.Text + ";;" + txtCode1.Text.Trim() + "|" + txtCode1.Text.Trim() + ";" + txtOpCode.Text.Trim() + ";#;" + NGState + ";" + NGDesc + "}";
                    }
                    break;
            }
            return sendString;
