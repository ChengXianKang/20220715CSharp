...
namespace MDC
{
    public partial class frmTpTest : Form
    {
        #region 私有字段
        /// <summary>
        /// 数据上传客户端
        ...
        public frmTpTest()
        {
            ...
            txtGlass1.Dock = DockStyle.Fill;
            txtGlass2.Dock = DockStyle.Fill;
            txtGlass3.Dock = DockStyle.Fill;
            ...
            this.txtGlass = new List<TextBox>() { txtGlass1, txtGlass2, txtGlass3 };
        ...
        /// <param name="e"></param>
        private void frmTpTest_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frmTpTest_Shown(object sender, EventArgs e)
        {
            // 连接服务器
            ClientStart();
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frmTpTest_FormClosed(object sender, FormClosedEventArgs e)
        {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frmTpTest_SizeChanged(object sender, EventArgs e)
        {
            this.Refresh();
        }
        /// <summary>
        ...
        private void Initialize()
        {
            ...
            this.ipAddress = "172.20.21.164";   //CFOGTP测试
#endif
            ...
            for (int i = 0; i < fixturePort.Length; i++)
            {
                ...
                this.txtGlass[i].BackColor = Color.Lavender;
            ...
            foreach (string COM in this.lstPortName)
            {
                ...
                SerialPortUtil port = new SerialPortUtil(COM, SerialPortBaudRates.BaudRate_9600, Parity.None, SerialPortDatabits.EightBits, StopBits.One);
        ...
        private void BindProcessComboBox(string ProductionOrder, string LineCode)
        {
            try
            {
                ...
                //判断是否TP测试工序
                if(BaseUI.UI_SGXJOBCODE != "015" && BaseUI.UI_SGXJOBCODE != "56" && BaseUI.UI_SGXJOBCODE != "59")
                {
                    ShowResult(NoteState.NG, ipAddress, "当前站点不是TP测试岗位");
        ...
        private void PageInit()
        {
            ...
            try
            {
                ...
                if (this.dvProcess.Count > 0)
                {
                    ...
                    //this.scanType2 = "";
                    lblCode1.Text = "1#" + this.scanType1 + "编码:";
                    lblCode2.Text = "2#" + this.scanType1 + "编码:";
                    lblCode3.Text = "3#" + this.scanType1 + "编码:";
        ...
        private void Fixture_DataReceived(Utils.DataReceivedEventArgs e)
        {
            try
            {
                ...
                string data = Encoding.GetEncoding("GBK").GetString(e.BytesReceived); 
                    ...
                    else
                    {
                        ...
                        switch (rst.State)
                        {
                            ...
                            case NoteState.Success:
                                ...
                                break;
                            case NoteState.OK:
                                //if(cmbSerialPort.SelectedIndex == -1 || string.IsNullOrEmpty(cmbSerialPort.Text))
                                //{
                                //    ShowResult(NoteState.NG, txtFixtureCode.Text, "未找到治具串口!");
                                //    return;
                                //}
                                if (fixtureNo != -1)
                                {
                                    this.txtCode[fixtureNo].BackColor = Color.Lime;
                                    this.txtGlass[fixtureNo].BackColor = Color.Lime;
                                    this.lblInfo[fixtureNo].Text = ProgressInfo.测试中.ToString();
                                }
                                this.lstBgk[fixtureNo].RunWorkerAsync(fixtureNo);
                                //this.bgkFixture.RunWorkerAsync(cmbSerialPort.Text);
        ...
        private void BgkFixture_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            byte[] data = Encoding.GetEncoding("GBK").GetBytes("按开关键\r\n");
            port.WriteData(data);
            //port.WriteData("按开关键\r\n");
            bgk.ReportProgress(10, fixtureNo);

            PublicSub.Delay(1000);
        ...
        private void BgkFixture_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            ...
            if (i == 10)
            {
                AddNote(NoteState.Success, "按开关键\r\n", portName + "发送数据，治具:" + fixtureCode);
            }
            else if (i == 20)
            {
        ...
        private void FixtureDataHandler(string PortName, string DataString)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (DataString == "END\r\n ")
                    {
                        AddNote(NoteState.Success, DataString, PortName + "接收数据，治具" + ": " + fixtureCode);
                    }
                    //TP测试功能:开
                    else if (DataString.Contains("TP测试功能:开"))
                    {
                        AddNote(NoteState.Success, DataString, PortName + "接收数据，治具" + ": " + fixtureCode);
                    }
                    //TP测试超时
                    else if (DataString.Contains("TP测试超时"))
                    {
                        if (fixtureNo != -1)
                        {
                            this.lblInfo[fixtureNo].Text = ProgressInfo.超时.ToString();
                            this.lblInfo[fixtureNo].ForeColor = Color.Red;
                        }
                        ShowResult2(NoteState.Error, DataString, PortName + "接收数据，治具" + ": " + fixtureCode, fixtureNo);
                        PublicSub.Delay(1000);
                        byte[] data = Encoding.GetEncoding("GBK").GetBytes("关机\r\n");
                        //this.fixturePortList[PortName].WriteData(data);
                        this.lstFixturePort[fixtureNo].WriteData(data);
                        AddNote(NoteState.Success, "关机", PortName + "发送数据，治具" + ": " + fixtureCode);
                    }
                    //TP测试FAIL
                    else if (DataString.Contains("TP测试FAIL"))
                    {
                        AddNote(NoteState.Success, DataString, PortName + "接收数据，治具" + ": " + fixtureCode);
                        if(fixtureNo != -1)
                        {
                        ...
                        // 生成发送字符串
                        string sendCode = GetSendString("1", "TP_NG", fixtureNo);
                        if (sendCode == "")
                        {
                    ...
                    //TP测试PASS
                    else if (DataString.Contains("TP测试PASS"))
                    {
                        AddNote(NoteState.Success, DataString, PortName + "接收数据，治具" + ": " + fixtureCode);
                        if (fixtureNo != -1)
                        {
        ...
        private NoteResult isValidFixtureCode(string DataString, out int fixtureNo)
        {
            ...
            if (!DataString.Contains("-ASSYTP-") && !DataString.Contains("-CFOGTP-"))
            {
                IsScan = false;
        ...
        private NoteResult isValidCode1(string DataString, int FixtureNo)
        {
            ...
            //不匹配Code1编码规则
            if (DataString.Substring(0, DataString.Length >= txtRule1.Text.Length ? txtRule1.Text.Length : DataString.Length) != txtRule1.Text && (DataString.Contains("-ASSYTP-") || DataString.Contains("-CFOGTP-")))
            {
                    IsScan = false;
            ...
            if (this.glassInfo.ProductInfo != null)
            {
                ...
                this.txtGlass[FixtureNo].Text = this.glassInfo.ProductInfo.LCDCode;//玻璃码
        ...
        private string GetSendString(string NGState, string NGDesc)
        {
            ...
            string sendString = "RGJT{RGZD;" + txtFixtureCode.Text + ";" + txtProcessIP.Text + ";" + txtGlass1.Text.Trim() + ";" + txtCode1.Text.Trim() + "|" + txtCode1.Text.Trim() + ";" + txtOpCode.Text.Trim() + ";" + NGState + ";" + NGDesc + "}";
            // 过站补扫点
        ...
        private string GetSendString(string NGState, string NGDesc, int FixtureNo)
        {
            ...
            try
            {
                ...
                glassCode = this.txtGlass[FixtureNo].Text.Trim();
